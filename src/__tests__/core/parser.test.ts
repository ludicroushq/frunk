...
describe("Parser", () => {
  describe("parseCommand", () => {
    ...
    it("throws error for nested frunk commands", () => {
      expect(() => {
        parseCommand(["[test]", "--", "f", "[build]", "--", "echo", "done"]);
      }).toThrow("Nested frunk commands are not supported");

      expect(() => {
        parseCommand(["[test]", "--", "frunk", "[build]"]);
      }).toThrow("Nested frunk commands are not supported");
    });

    it("allows non-frunk commands after --", () => {
      const result = parseCommand(["[test]", "--", "npm", "run", "build"]);
